...
CREATE TABLE `matches` (
	...
	`is_completed` integer DEFAULT false,
	`is_abandoned` integer DEFAULT false,
	`is_tied` integer DEFAULT false,
	`margin` text,
	`player_of_the_match_id` integer,
	`has_lbw` integer DEFAULT false,
	`has_bye` integer DEFAULT false,
	`has_leg_bye` integer DEFAULT false,
	`has_boundary_out` integer DEFAULT false,
	`has_super_over` integer DEFAULT false,
	`venue_id` integer,
	`notes` text,
	...
	FOREIGN KEY (`player_of_the_match_id`) REFERENCES `players`(`id`) ON UPDATE no action ON DELETE no action,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `unique_team_player` ON `team_players` (`team_id`,`player_id`);--> statement-breakpoint
CREATE TABLE `teams` (
	`id` integer PRIMARY KEY NOT NULL,
	`name` text NOT NULL,
